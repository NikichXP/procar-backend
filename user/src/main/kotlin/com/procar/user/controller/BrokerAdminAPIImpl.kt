...
class BrokerAdminAPIImpl(private val brokerService: BrokerService) : BrokerAdminAPI {
    ...
    override suspend fun updateBrokerStatus(id: String, request: UpdateBrokerStatusRequest): BrokerDto {
        return brokerService.updateBrokerStatus(id, request.status)
            ?: throw ResponseStatusException(HttpStatus.NOT_FOUND, "Broker not found")
    }

    override suspend fun patchBroker(id: String, request: PatchBrokerRequest): BrokerDto {
        return brokerService.patchBroker(id, request)
            ?: throw ResponseStatusException(HttpStatus.NOT_FOUND, "Broker not found")
    }
